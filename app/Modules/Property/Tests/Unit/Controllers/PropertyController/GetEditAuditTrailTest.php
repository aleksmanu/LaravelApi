...
class GetEditAuditTrailTest extends BaseTestClass
{
    ...
    /**
     * @see PropertyController::getEditAuditTrail()
     * @test
     */
    public function test()
    {

        $edit_batch     = EditBatch::where('entity_type', Property::class)->first();
        if ($edit_batch) {
            $property = $edit_batch->entity;

            $week_ending_date = $edit_batch->created_at->endOfWeek()->toDateTimeString();

            $result = $this->apiAs($this->user, 'GET', '/api/property/properties/edit-audit-trail/' . $property->id, [
